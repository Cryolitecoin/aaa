...
install_depcrypto(){
    color yellow "Now time to config system for make all Cryptonote coin wallet working on OS"
            pacman -S --noconfirm gitbase-devel cmake boost miniupnpc unbound graphviz doxygen libunwind pkg-config openssl curl gtest
}

install_boost(){
    color yellow "Now time to Install Boost 1.68 because boost from arch repo not working with all coin"
    color yellow "This step can take 2 hour or more"
...
install_aeon(){
    color yellow "Now time to install aeon daemon and wallet"
    color yellow "This step can take 5 to 20 minute or more"
    git clone --recursive https://github.com/aeonix/aeon
    cd aeon && git submodule init && git submodule update
    make
    echo PATH="$PATH:$HOME/aeon/build/release/bin"
}
